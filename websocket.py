...
def recvPackets(self, sock):
    ...
    rem_packets = packets
    now_time = time.time()
    print "time taken to recieve packets", now_time - cur_time
    while rem_packets and flag:
        sock.sendto(bytearray(randomword(list[packets- rem_packets])), sending_addr)
        rem_packets-=1
        print 'sending packetss', packets - rem_packets

    print 'the time taken to send packets', time.time() - now_time
...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    def open(self):
        ...
        message = {'code': 201, 'body': 'you are connected'}
        self.write_message(json.dumps(message))
    def on_message(self,message):
        ...
        try:
            ...
            print 'error',e 
            print_msg(self, 400, 'bad message')
        #except:
    ...
    def client_start(self, message):
        ...
        '''
             ...
             print "server is busy this time"
             print_msg(self, 502, 'server busy')
             try:
                self.close()
             except:
